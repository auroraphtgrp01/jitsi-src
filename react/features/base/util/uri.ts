 ...
 * incompatible with Hoc Lien Tuc on the client and/or server sides. The main
...
export function parseStandardURIString(str: string) {
    ...
    // whitespace. Anyway, the Hoc Lien Tuc app is not known to utilize unencoded
...
/**
 * Parses a specific URI which (supposedly) references a Hoc Lien Tuc resource
 * (location).
 *
 * @param {(string|undefined)} uri - The URI to parse which (supposedly)
 * references a Hoc Lien Tuc resource (location).
 * @public
 * @returns {{
 *     contextRoot: string,
 *     hash: string,
...
export function parseURIString(uri?: string): any {
    ...
    // XXX While the components/segments of pathname are URI encoded, Hoc Lien Tuc
    ...
    // Add the properties that are specific to a Hoc Lien Tuc resource (location)
